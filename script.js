...
function initGame(){
    ...
    if(m){
        ...
        function debouncedUpdateSkins() {    
            clearTimeout(skinUpdateTimeout);    
            skinUpdateTimeout = setTimeout(updateSkinHighlights, 150);
        }
        ...
        showToast=t=>{
            ...
            tc++;
            ot(t);
            setTimeout(()=>{tc=Math.max(0,tc-1);},3e3);
        };

        // РЕВЕРБ СИСТЕМА - ОПТИМІЗАЦІЯ ОНОВЛЕННЯ
        const or=updateReverbClockHands;
        let lr=0;
    ...
    // === ОНОВЛЕННЯ ДАТИ ===
    function updateDate(){
        id("currentDate").textContent=new Date().toLocaleDateString('uk-UA');
    }
    updateDate();
    ...
    function formatTime(s){
        s=Math.floor(s);
        const u=[{name:"століття",v:3153600000},{name:"десятиліття",v:315360000},{name:"рік",v:31536000},{name:"міс",v:2592000},{name:"дн",v:86400},{name:"год",v:3600},{name:"хв",v:60},{name:"сек",v:1}];
        let r=s,p=[];
        for(const x of u){
    ...
    let timeCache = {};
    function memoizedFormatTime(s) {    
        if(timeCache[s]) return timeCache[s];    
        const result = formatTime(s);    
        if(Object.keys(timeCache).length > 50) timeCache = {};    
        return timeCache[s] = result;
    }
    ...
    function updatePrestigeProgress() { 
        ...
        const pB = id('prestigeProgressBar'), 
              pT = id('prestigeProgressText'), 
              pP = Math.min(100, (currentPrestigeProgress / prestigeThreshold) * 100);
        
        if(pB) { 
            pB.style.width = pP + '%'; 
            pB.style.background = pP >= 100 ? 'linear-gradient(90deg, #10b981, #34d399)' : pP >= 50 ? 'linear-gradient(90deg, #f59e0b, #fbbf24)' : 'linear-gradient(90deg, #ef4444, #f87171)'; 
        }
    ...
    function updateReverbText() { 
        nextMultiplierEl.textContent = (prestigeMultiplier * 1.2).toFixed(2); 
        const rD = id('reverbDesc'); 
        if(rD) rD.textContent = `Потрібно для перезапуску: ${formatTime(prestigeThreshold)}`; 
    }
    ...
    const clockSkins=[
        ...
        shapes=[
            {id:"round",n:"Круг",p:0},
            {id:"square",n:"Квадрат",p:28800},
            {id:"diamond",n:"Ромб",p:86400},
            {id:"oval",n:"Овал",p:172800}
        ],
        handSkins=[
        ...
        effects=[
            {id:"red",n:"Червоний спалах",p:0},
            {id:"blue",n:"Синій вибух",p:21600},
            {id:"glitch",n:"Глітч",p:108000},
            {id:"blackhole",n:"Чорна діра",p:360000},
    ...
    const restartEffect={
        ...
        init(){
            ...
            this.clocksContainer=document.createElement("div");
            ...
            this.completionScreen.innerHTML=`<div style="font-size:32px;margin-bottom:20px;text-shadow:0 0 20px #0ea5e9">Ви успішно повернулися в часі!</div><div style="font-size:24px;margin-bottom:20px">Ваш множник: ${prestigeMultiplier.toFixed(2)}×</div><div style="font-size:18px;color:#cfeaff">Натисніть будь-де щоб продовжити</div>`;
            document.body.appendChild(this.completionScreen);
        },
        start(){
            this.active=!0;
            document.getElementById("restartEffectOverlay").style.display="block";
        ...
        stop(){
            ...
            document.getElementById("restartEffectOverlay").style.display="none";
        },
        ...
        createFlyingClock(){
            ...
            Math.random()>0.5&&(e.style.transform="scaleX(-1)");
            this.clocksContainer.appendChild(e);
            setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},1e4);
        },
        createStatBubble(){
            const e=document.createElement("div");
            ...
            setTimeout(()=>{e.parentNode&&e.parentNode.removeChild(e)},4e3);
        }
    };
    restartEffect.init();
    ...
    function updateReverbClockHands(){
        ...
        const t=new Date(),
              o=t.getSeconds()+t.getMilliseconds()/1000,
              n=t.getMinutes()+o/60,
              a=(t.getHours()%12||12)+n/60;
    ...
    const startReverbHold=e=>{
        ...
        qa("#reverbClock .hand").forEach((e,t)=>{
            const o=0.5+2*Math.random(),
                  n=Math.random()>0.5?"normal":"reverse";
