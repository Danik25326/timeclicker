...
// === СИСТЕМА КЛІКІВ ===
function addTime(){const g=Math.round(clickPower*clickMultiplier*prestigeMultiplier);score+=g;clickCloudTotal+=g;if(g>maxPerClick)maxPerClick=g;clickGainEl.textContent=`+${formatTime(g)}`;showFloating(`+${formatTime(g)}`);triggerClickEffect();handleClickCombo();}
...
// === РЕВЕРБ СИСТЕМА ===
rb.addEventListener("click",()=>{if(!confirm("Повернути час назад? Втратиш апгрейди, отримаєш множник!"))return;srm();});
function srm(){ro.classList.remove("hidden");tt.classList.add("active");rh.style.opacity="1";ira=1;rc.className=`clock ${cr.shape}`;cs.find(s=>s.id===cr.clock)?.a();hs.find(s=>s.id===cr.hand)?.a();urch();setTimeout(()=>rh.style.opacity="0",3e3);}
function urch(){ // ОНОВЛЕННЯ СТРІЛОК РЕВЕРБ ГОДИННИКА
    if(!ira)return;const h=qa("#reverbClock .hand");if(h.length===0)return;
    if(rc.classList.contains("reverb-chaos")){requestAnimationFrame(urch);return;} // ЯКЩО ХАОС - ВИХОДИМО
    const n=new Date(),s=n.getSeconds()+n.getMilliseconds()/1e3,m=n.getMinutes()+s/60,h=(n.getHours()%12||12)+m/60; // НОРМАЛЬНИЙ ЧАС
    ...
    qa("#reverbClock .hour").forEach(x=>x.style.transform=`translateX(-50%)rotate(${h*30}deg)`);requestAnimationFrame(urch);
}
const srh=e=>{ // ПОЧАТОК УТРИМАННЯ
    if(e.type.includes('touch'))e.preventDefault();if(!ira)return;
    rh.style.opacity="0";rc.classList.add("reverb-mode","reverb-chaos");tt.classList.add("intense");
    qa("#reverbClock .hand").forEach(h=>{ // ХАОТИЧНЕ ОБЕРТАННЯ
        const d=0.5+Math.random()*2,dir=Math.random()>0.5?"normal":"reverse"; // ВИПАДКОВА ШВИДКІСТЬ/НАПРЯМОК
        h.style.setProperty("--duration",`${d}s`);h.style.setProperty("--direction",dir);h.style.animation=`chaosSpin ${d}s linear infinite ${dir}`;
    });rht=setTimeout(cr,1e4);
};
const strh=e=>{ // ЗУПИНКА УТРИМАННЯ
    if(e&&e.type.includes('touch'))e.preventDefault();clearTimeout(rht);
    if(ira){rc.classList.remove("reverb-mode","reverb-chaos");tt.classList.remove("intense");
        qa("#reverbClock .hand").forEach(h=>{h.style.animation="none";h.style.removeProperty("--duration");h.style.removeProperty("--direction");});urch();} // ПОВЕРТАЄМО ЧАС
};
rc.addEventListener("mousedown",srh);rc.addEventListener("touchstart",srh,{passive:false});
rc.addEventListener("mouseup",strh);rc.addEventListener("mouseleave",strh);
rc.addEventListener("touchend",strh);rc.addEventListener("touchcancel",strh);
function cr(){ // ЗАВЕРШЕННЯ РЕВЕРБУ
    strh(new Event('manual'));pm*=1.2;tr++;s=0;cp=1;ar=0;tub=0;mpc=1;cct=0;ccb=0; // СКИДАЄМО ПРОГРЕС
    ug.forEach((u,i)=>{u.l=0;if(btns[i]){btns[i].classList.add("hidden");if(i===0)btns[i].classList.remove("hidden");}u.up();}); // СКИДАЄМО АПГРЕЙДИ
    mls.forEach(m=>{if(m.b){m.b=0;cm=1;}m.up&&m.up();}); // СКИДАЄМО МНОЖНИКИ
    tt.classList.add("reverb-complete");setTimeout(()=>{
        alert(`Перезапуск завершено! Множник: ${pm.toFixed(2)}×`);
        ro.classList.add("hidden");tt.classList.remove("active","intense","reverb-complete");ira=0;us();ust();ua(); // ОНОВЛЮЄМО ІНТЕРФЕЙС
    },1500);
}
// === СИСТЕМА ВКЛАДОК ===
